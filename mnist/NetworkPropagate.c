...
static void mac4_soft(const UDATA_T* __restrict inputs,
                 const WDATA_T* __restrict weights,
                 SUM_T* __restrict weightedSum,
                 int nb_iterations)
{
    ...
    for (; iter <= nb_iterations - 4; iter += 4) {
        uint32_t val_in = ((uint32_t)inputs[iter]) |
                          ((uint32_t)inputs[iter+1] << 8) |
                          ((uint32_t)inputs[iter+2] << 16) |
                          ((uint32_t)inputs[iter+3] << 24);

        // Note: weights may be signed, cast to uint8_t first to avoid sign extension interference in concatenation
        uint32_t val_wt = ((uint32_t)(uint8_t)weights[iter]) |
                          ((uint32_t)(uint8_t)weights[iter+1] << 8) |
                          ((uint32_t)(uint8_t)weights[iter+2] << 16) |
                          ((uint32_t)(uint8_t)weights[iter+3] << 24);
...
static void mac4(const UDATA_T* __restrict inputs, const WDATA_T* __restrict weights, SUM_T* __restrict weightedSum, int nb_iterations)
{
    // 1. Performance optimization: load accumulation value from memory into CPU local variable (register)
    int32_t sum = *weightedSum;
    int iter = 0;

    // 2. Core acceleration loop: process 4 elements at a time (SIMD concept)
    for (; iter <= nb_iterations - 4; iter += 4) {
        // Get the memory start address of current 4 elements
        const UDATA_T *p_in = inputs + iter;
        const WDATA_T *p_wt = weights + iter;

        // Call the dot8 hardware instruction       
        asm volatile (
        "lw t1, 0(%[p_in]) \n\t" // Load 4 8-bit inputs at once into 32-bit value t1
        "lw t2, 0(%[p_wt]) \n\t" // Load 4 8-bit weights at once into 32-bit value t2
        "dot8 %[sum], t1, t2 \n\t" // Perform dot product accumulation: sum = sum + (t1 · t2)
        : [sum] "+r" (sum) // Output+Input: accumulator sum is both destination and source operand
        : [p_in] "r" (p_in), // Input: pointer to inputs
        [p_wt] "r" (p_wt) // Input: pointer to weights
        : "t1", "t2", "memory" // Clobber list: tell compiler we temporarily use t1 and t2 registers
        );
    }

    // 3. Tail cleanup (Remainder Loop): process remaining elements that are not divisible by 4
    for (; iter < nb_iterations; ++iter) {
        sum += inputs[iter] * weights[iter];
    }

    // 4. Write final accumulation result back to memory
    *weightedSum = sum;
}
